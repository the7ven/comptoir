    ...
    <div className="fade-in text-left">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div className="text-left">
          <h3 className="text-3xl font-black italic tracking-tighter uppercase text-left">
            Plan de Salle
          </h3>
          <p className="opacity-50 text-[10px] font-black uppercase tracking-widest text-left">
            {tables.length} Tables en gestion
          </p>
        </div>
        <button
          onClick={() => setIsAddTableModalOpen(true)}
          className="flex items-center gap-2 bg-[#00D9FF] text-black px-6 py-4 rounded-2xl font-black text-xs uppercase shadow-lg active:scale-95 transition-all"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {tables.map((table) => {
          const tableOrders = activeOrders.filter(
            (o) => o.table_number === table.table_name,
          );
          const currentStatus =
            tableOrders.length > 0
              ? tableOrders.some((o) => o.status === "Prêt")
                ? "Addition"
                : "Occupée"
              : "Libre";
          ...
          return (
            ...
            >
              <button
                ...
                className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 p-2 text-red-500 transition-all border-none bg-transparent cursor-pointer"
              >
                <Trash2 size={16} />
              </button>
              <div
                className={`w-14 h-14 rounded-full flex items-center justify-center border ${currentStatus === "Libre" ? "border-dashed border-current/20" : "border-current"}`}
              >
                <Grid size={24} />
              </div>
      ...
      {isAddTableModalOpen && (
        <div className="fixed inset-0 z-[700] flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
          <div
            className={`relative w-full max-w-sm p-10 rounded-[45px] border shadow-2xl ${isDarkMode ? "bg-[#0a0a0a] border-white/10" : "bg-white border-gray-200"}`}
          >
            <button
              onClick={() => setIsAddTableModalOpen(false)}
              className="absolute top-6 right-6 p-2 opacity-50 hover:opacity-100 text-white border-none bg-transparent cursor-pointer"
            >
              <X size={24} />
            </button>
            <form onSubmit={handleAddTable}>
              <h3 className="text-xl font-black mb-8 italic uppercase text-white">
                Créer une table
              </h3>
              <div className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    type="text"
                    inputMode="numeric"
                    placeholder="Tapez juste le numéro (ex: 7)"
                    className={`w-full mt-2 px-6 py-4 rounded-2xl border outline-none ${isDarkMode ? "bg-white/5 border-white/10 text-white" : "bg-gray-50"}`}
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase opacity-40 ml-4">
                ...
                <button
                  type="submit"
                  className="w-full py-5 bg-[#00D9FF] text-black font-black rounded-2xl uppercase text-[10px] tracking-widest mt-4 border-none cursor-pointer"
                >
                  Valider
                </button>
              </div>
      ...
      {multiOrderTable && (
        <div className="fixed inset-0 z-[700] flex items-center justify-center p-4 backdrop-blur-md bg-black/60">
          <div
            className={`relative w-full max-w-md p-8 rounded-[40px] border shadow-2xl ${isDarkMode ? "bg-[#0a0a0a] border-white/10" : "bg-white border-gray-200"}`}
          >
            <button
              onClick={() => setMultiOrderTable(null)}
              className="absolute top-6 right-6 opacity-50 hover:opacity-100 text-white border-none bg-transparent cursor-pointer"
            >
              <X size={24} />
            </button>
            <h3 className="text-xl font-black mb-6 italic uppercase">
              Factures : {multiOrderTable.name}
            </h3>
              ...
              {multiOrderTable.orders.map((order, idx) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    setMultiOrderTable(null);
                  }}
                  className={`flex items-center justify-between p-5 rounded-3xl border cursor-pointer transition-all ${isDarkMode ? "bg-white/5 border-white/5 hover:border-[#00D9FF]" : "bg-gray-50 hover:border-cyan-500"}`}
                >
                  <div className="text-left">
                    ...
                    <p className="text-[9px] opacity-40 font-bold">
                      {order.items_summary?.substring(0, 30)}...
                    </p>
                  </div>
    ...
    <div className="w-full max-w-sm">
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 z-[900] flex items-center justify-center p-4 backdrop-blur-md bg-black/40 text-center">
          <div
            className={`w-full max-w-sm rounded-[40px] p-8 border shadow-2xl ${isDarkMode ? "bg-[#0f0f0f] border-white/10 text-white" : "bg-white border-gray-200"}`}
          >
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-4 rounded-2xl font-black text-xs uppercase bg-red-500 text-white border-none cursor-pointer"
              >
                Confirmer
              </button>
            </div>
