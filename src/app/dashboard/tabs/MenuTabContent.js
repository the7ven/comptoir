...
import { 
  ...
  Minus, RotateCcw, Utensils, Beer, List, Search, Printer
} from 'lucide-react';
...
export default function MenuTabContent({ isDarkMode, cart, setCart, setActiveTab, pendingOrder, setPendingOrder }) {
  ...
  // --- LOGIQUE DE FILTRAGE (PLATS + ACCOMPAGNEMENTS DANS LE MÊME ONGLET) ---
  ...
  // --- IMPRESSION TICKET PROVISOIRE ---
  const printProvisionalTicket = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <title>Note - ${tableNum || 'Comptoir'}</title>
          <style>
            body { font-family: 'Courier New', Courier, monospace; width: 280px; padding: 10px; }
            .header { text-align: center; border-bottom: 1px dashed #000; padding-bottom: 10px; margin-bottom: 10px; }
            .item { display: flex; justify-content: space-between; margin-bottom: 5px; font-size: 13px; }
            .total { border-top: 2px solid #000; margin-top: 10px; padding-top: 5px; font-weight: bold; font-size: 16px; display: flex; justify-content: space-between; }
            .footer { text-align: center; margin-top: 15px; font-size: 10px; opacity: 0.7; }
          </style>
        </head>
        <body>
          <div class="header">
            <h3 style="margin:0">NOTE PROVISOIRE</h3>
            <p style="margin:5px 0">${orderType === "Sur place" ? 'TABLE: ' + tableNum : 'A EMPORTER'}</p>
            <p style="font-size:10px">${dateStr}</p>
          </div>
          ${cart.map(item => `<div class="item"><span>${item.quantity}x ${item.name}</span><span>${(item.price * item.quantity).toLocaleString()} F</span></div>`).join('')}
          <div class="total"><span>TOTAL</span><span>${total.toLocaleString()} F</span></div>
          <div class="footer"><p>Document non fiscal<br/>RestoPay Cloud</p></div>
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 no-print min-h-screen">
      ...
      {/* --- PANIER STICKY (Fixé à gauche au scroll) --- */}
      <div className={`w-full lg:w-96 flex flex-col rounded-[35px] lg:rounded-[45px] border transition-all shrink-0 lg:sticky lg:top-8 h-fit max-h-[calc(100vh-100px)] ${isDarkMode ? 'bg-[#0a0a0a] border-white/5' : 'bg-white border-gray-100 shadow-xl'}`}>
        ...
        <div className="overflow-y-auto p-4 space-y-3 custom-scrollbar flex-1">
          {cart.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center opacity-20 italic text-sm">
              <Plus size={40} className="mb-2" />
              <p>Panier vide</p>
            </div>
          ) : (
            cart.map((item) => (
              <div key={item.id} className={`p-4 rounded-2xl flex items-center justify-between animate-in fade-in ${isDarkMode ? 'bg-white/5' : 'bg-gray-50'}`}>
        ...
        {cart.length > 0 && (
          <div className="p-6 border-t border-white/5 space-y-4">
            ...
            <div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl border border-white/5">
              <div>
                <p className="text-[9px] font-black opacity-40 uppercase tracking-widest text-left">Total</p>
                <p className="text-xl font-black italic text-[#00D9FF]">{cart.reduce((a, b) => a + (b.price * b.quantity), 0).toLocaleString()} F</p>
              </div>
              <button onClick={printProvisionalTicket} className="p-3 rounded-xl bg-white/5 text-white/50 hover:text-white transition-all"><Printer size={20}/></button>
            </div>
            <button onClick={finalizeOrder} className="w-full py-4 bg-[#00D9FF] text-black rounded-2xl font-black text-[10px] uppercase shadow-lg shadow-cyan-500/20 active:scale-95 transition-all">Valider la commande</button>
          </div>
        )}
      </div>

      {/* --- GRILLE DU MENU (SCROLL NATUREL) --- */}
      <div className="flex-1 w-full text-left">
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-10 gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
            ...
            <button onClick={() => { setEditingItem(null); setIsModalOpen(true); }} className="bg-[#00D9FF] text-black px-8 py-4 rounded-2xl font-black text-xs uppercase shadow-lg flex items-center justify-center gap-3">
            ...
            <div key={item.id} className={`group relative rounded-[35px] overflow-hidden border transition-all duration-500 ${isDarkMode ? 'bg-[#0a0a0a] border-white/5 hover:border-[#00D9FF]/30' : 'bg-white border-gray-100 shadow-sm'}`}>
              <div className="h-44 relative overflow-hidden">
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-3 backdrop-blur-sm">
                  <button onClick={() => addToCart(item)} className="w-12 h-12 bg-[#00D9FF] text-black rounded-2xl flex items-center justify-center shadow-2xl hover:scale-110 transition-transform"><Plus size={24} strokeWidth={3} /></button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 backdrop-blur-xl bg-black/60">
          <form onSubmit={handleSaveDish} className={`w-full max-w-lg rounded-[45px] p-10 border shadow-2xl ${isDarkMode ? 'bg-[#0a0a0a] border-white/10 text-white' : 'bg-white border-gray-100 text-gray-900'}`}>
            <h3 className="text-2xl font-black mb-8 italic uppercase text-left">{editingItem?.id ? "Modifier" : "Nouveau Produit"}</h3>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select name="category" defaultValue={editingItem?.category || "Plats"} className={`w-full px-6 py-4 rounded-2xl border outline-none ${isDarkMode ? 'bg-[#151515] border-white/10' : 'bg-gray-50'}`}>
                  <option value="Plats">Plat principal / Accompagnement</option>
                  <option value="Boissons">Boisson (Bar)</option>
